...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if attn_mask is not None:
            attn_mask = einops.rearrange(attn_mask, '(b h) j k -> b h j k', b=B, h=self.num_heads)
            attn = attn_mask + (q @ k.transpose(-2, -1)) * self.scale
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward_head(self, x, pre_logits: bool = False, is_san=False):
        # add is_san for SAN setting.
        if is_san:
            return self.head(x)
        if self.global_pool:
...
def resize_pos_embed(posemb, posemb_new, num_prefix_tokens=1, gs_new=()):
    ...
    posemb_grid = F.interpolate(posemb_grid, size=gs_new, mode='bicubic', align_corners=True)
